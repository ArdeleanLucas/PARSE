...
// to: backend`). If that copy drops the executable bit on the frozen backend
...
// Must stay in sync with electron-builder.yml `extraResources: to: backend` and
// backend-launcher.js's FROZEN_BACKEND_DIR / frozenBackendExeName.
...
// electron-builder.yml (`to: backend`) + the PyInstaller onedir layout, and is
// the same path backend-launcher.js resolves at runtime.
  ...
  if (!fs.existsSync(exePath)) {
    throw new Error(
      `[afterPack] frozen PARSE backend not found at ${exePath}. ` +
        ...
        `to backend) to have copied the PyInstaller onedir output. Did the ` +
        `freeze step run before packaging?`
    );
  }
