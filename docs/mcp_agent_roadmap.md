...
- **60** in-app `ParseChatTools`
- **60** default MCP task tools
- **3** workflow macros
- **64** tools on the default adapter surface including `mcp_get_exposure_mode`
- **64** tools on the full adapter surface when `expose_all_tools=true`
- **44** tools on the legacy curated opt-out surface when `expose_all_tools=false` is configured explicitly
...
**Shipped on `main`:**
- `python/adapters/mcp_adapter.py` (thin MCP entrypoint; concrete adapter modules live under `python/adapters/mcp/`) exposes the shipped **61-tool** default task surface from `DEFAULT_MCP_TOOL_NAMES`.
...
**Current totals:**
- default adapter surface: **65** tools total
- legacy curated opt-out surface: **45** tools total
- full adapter surface: **65** tools total
