...
- `python/ai/chat_tools.py` is now **3192 LoC** on rebuild `origin/main`.
- `python/adapters/mcp_adapter.py` remains **2050 LoC** and untouched.
...
|---|---:|---|
| `python/server.py` | 7757 | still the largest backend monolith |
| `python/ai/chat_tools.py` | 3192 | materially reduced; final extraction wave still queued |
| `src/ParseUI.tsx` | 2035 | already structurally cracked |
| `python/adapters/mcp_adapter.py` | 2050 | untouched; PR 1 seam queued |
| `python/ai/provider.py` | 1907 | untouched |
...
- current exact line map in `python/ai/chat_tools.py`
