...
Single-interval lexeme rerun endpoints (`/api/lexeme/run_ortho`, `/api/lexeme/run_ipa`) are synchronous, speaker-lock-protected helpers for reviewer-driven correction. The tagged batch endpoint (`/api/lexemes/rerun-by-tag`) is job-tracked by default and should be polled through the returned `jobId`; set `async=false` only for deprecated compatibility with the legacy synchronous result shape. All rerun paths validate speaker/concept existence, reject intervals longer than 60 seconds, widen only the acoustic window by `pad`, and report the original selected `start`/`end` in per-interval responses so callers do not accidentally retime data just because they requested more acoustic context.
