...
| `POST /api/lexemes/rerun-by-tag` | Run ORTH/IPA/both per concept matched by tags, sequentially | Body: `{ tagLabels[], field: "ipa"\|"ortho"\|"both", match?, pad? \(0.0\|0.2\|0.5\), speakers? }`; loops the matched concept windows through the same speaker-locked rerun path as `/api/lexeme/run_*`; ambiguous labels return 409 to avoid spending GPU on an unapproved set |
...
`POST /api/lexemes/rerun-by-tag` reuses the same resolution logic, then loops the matched concept windows through the existing per-interval ORTH/IPA rerun path (`/api/lexeme/run_*`). Each per-concept call respects the same speaker lock and 60-second window cap as the per-interval endpoints. Per-concept errors are collected into `results[*]` (with `status: "error"`, the original HTTP `statusCode`, and a message) without aborting the batch. Ambiguous labels return HTTP 409 before any rerun runs so the endpoint never spends GPU on a concept set the caller did not explicitly approve.
...
- `default` — force the shipped default 62-tool surface
- `all` — force the full tool surface (currently also 62 tools unless a future explicit all-only surface diverges)
...
## Legacy curated MCP task surface (38 `ParseChatTools` + 3 workflow macros; 42 adapter tools including `mcp_get_exposure_mode`)
...
The shipped MCP default exposes the full safe 58-tool PARSE task surface plus 3 high-level workflow macros and `mcp_get_exposure_mode`; `expose_all_tools=false` is the legacy curated opt-out.
