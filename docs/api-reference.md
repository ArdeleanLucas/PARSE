...
| `GET /api/jobs/active` | List active and recently terminal backend jobs | Used to rehydrate in-flight progress after reload and briefly expose terminal complete/error/cancelled snapshots for the header strip |
...
| `DELETE /api/concepts/{conceptId}` | Delete one `concepts.csv` row | Returns 409 with `blocking_speakers` when any speaker has annotated the row; `?cascade=true` first purges those recordings (concept + IPA/ortho/ortho_words/speaker rows at the same time range) from every live `{speaker}.parse.json`, backs each file up, deletes the row, and returns `purged_intervals` + `purged_speakers` |
...
| `POST /api/concepts/by-tag` | List concepts carrying selected tags, grouped by speaker | Body: `{ tagLabels[], match? = "any" \| "all", speakers? = "all" \| string[] }`; returns `{ totalConcepts, perSpeaker, unknownTags, ambiguousTags }` without mutating state |
| `POST /api/lexemes/rerun-by-tag` | Start an ORTH/IPA/both rerun compute job for concepts matched by tags | Body: `{ tagLabels[], field: "ipa"\|"ortho"\|"both", match?, pad? \(0.0\|0.2\|0.5\), speakers?, async? }`; default `async=true` returns `202 + jobId` for polling; `async=false` is deprecated synchronous compatibility; ambiguous labels return 409 before any job or GPU work starts |
| `POST /api/tags/import` | Import tags from CSV | Multipart form upload |
| `POST /api/lexeme/run_ortho` | Start a tracked ORTH rerun job for one selected lexeme interval | Body: `speaker`, `concept_key`, `start`, `end`, optional `pad` (`0.0`, `0.2`, `0.5`), optional `async`; default returns `202 + jobId`, while `async=false` keeps deprecated synchronous `{ ortho, interval, source: "rerun", jobId: null }` compatibility |
| `POST /api/lexeme/run_ipa` | Start a tracked IPA rerun job for one selected lexeme interval | Same body/pad/`async` contract as ORTH; default returns `202 + jobId`, while `async=false` keeps deprecated synchronous `{ ipa, interval, source: "rerun", jobId: null }` compatibility |
...
`DELETE /api/concepts/{conceptId}` deletes one canonical `concepts.csv` row (with a pre-delete `concepts.csv` backup). By default a row that any speaker has annotated is protected: the endpoint returns HTTP 409 with `blocking_speakers` — every speaker whose `concept` tier still references the row, read from the authoritative live `{speaker}.parse.json` (a stale legacy `{speaker}.json` is ignored when a `.parse.json` exists). Passing `?cascade=true` lifts that protection by first purging the blocking recordings — each `concept`-tier interval plus its sibling IPA/orthography/word-boundary/speaker rows at the same time range (0.001s tolerance, matching `POST /api/annotations/intervals/delete`) — from every live annotation file, backing each up as `{name}.bak-<UTC>-pre-purge-<conceptId>` before rewriting, then deletes the row. A cascade response adds `purged_intervals` (total recordings removed) and `purged_speakers`, which equals the set the 409 warning would list; speakers that never annotated the row are left untouched and are not backed up. Use cascade only to remove a junk variant and its recordings in one step.
...
| `lexeme_rerun_ipa` | `lexeme-rerun-ipa`, `lexeme_ipa`, `lexeme-ipa` | Run one selected lexeme IPA rerun as a tracked compute job |
| `lexeme_rerun_ortho` | `lexeme-rerun-ortho`, `lexeme_ortho`, `lexeme-ortho` | Run one selected lexeme ORTH rerun as a tracked compute job |
| `lexemes_rerun_by_tag` | `lexemes-rerun-by-tag`, `tagged_rerun`, `tagged-rerun` | Run ORTH/IPA/both over tag-matched concept windows as a tracked compute job |
