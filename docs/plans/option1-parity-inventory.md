...
| Annotate BND / phonetic-tools UI | P0 | `ported-awaiting-final-rerun` | `docs/reports/2026-04-27-pr149-scope-investigation.md` | PR #149 is the real frontend BND port: `phonetic-refine-boundaries`, `phonetic-retranscribe-with-boundaries`, and their gate tests are present on rebuild main. Target final status is `pass-via-harness` after the coordinator refreshes the stale exact-string source-audit rules and reruns the harness. |
| BND UI gate surfaces (`tiers.ortho_words`, STT word timestamps) | P0 | `ported-awaiting-final-rerun` | `docs/reports/2026-04-27-pr149-scope-investigation.md` | The actual gate logic is present on main via `bndIntervalCount` and `sttHasWordTimestamps`; the remaining harness diffs are false negatives caused by overly literal oracle-string matching. |
| BND / MCP tool surface | P1 | `ported-awaiting-final-rerun` | `docs/reports/2026-04-27-pr149-scope-investigation.md` | PR #152 landed the backend foundation (`boundaries`, `retranscribe_with_boundaries`, chat/MCP exposure). Target final status is `pass-via-harness` after a clean coordinator rerun on the refreshed feature-contract audit. |
