...
- Full-file, concept-window, and per-lexeme HF ORTH now suppress configured `ortho.decoding.initial_prompt` by default so short clips do not parrot the decoder prime. See [ORTH initial prompt suppression](./orth-initial-prompt-suppression.md) for the 2026-05-07 regression audit.
