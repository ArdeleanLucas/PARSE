...
| `concept_tags` | `Record<string, string[]>` keyed by concept id; values are tag ids | `src/api/types.ts`; `python/server_routes/annotate.py`; `src/stores/annotation/actions.ts` | Per-speaker tag membership for concepts. This replaces project-global tag membership for confirmation state so confirming Saha01 concept `1` does not mark Khan04 concept `1` as confirmed. Empty memberships normalize away rather than emitting a blank sidecar. | Shipped in the 2026-05-04 concept-tag PR cycle (#267/#268/#270) and speaker-scoped filter follow-up #284. |
