...
- **62** total default adapter tools including the 3 workflow macros plus `mcp_get_exposure_mode`
- **62** total adapter tools when `config/mcp_config.json` or `mcp_config.json` sets `{ "expose_all_tools": true }`
