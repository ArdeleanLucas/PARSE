...
- **Current target milestone:** Gate C — signing + notarization (issue #693). The Gate B packaging scope landed on main through PR #688.
- **Overall readiness (2026-08-28):** macOS packaging pipeline runs in CI (freeze → packaged-backend smoke → unsigned DMG); dependency lock committed; desktop runtime hardening (loopback bind, non-wildcard CORS, `/api/health`), project lifecycle, ffmpeg discovery, CPU-default compute, and the model registry + Settings Models manager + per-project binding are all merged (#671–#688).
- **Open blockers (2026-08-28):** signing/notarization credentials (C1, issue #693 — needs Lucas's Apple Developer identity); first-run STT fetch (frontend, see B3 packaging item); Windows packaging (NSIS) not started.
...
- [ ] Frozen per-platform Python runtime implemented (PyInstaller/Nuitka; no required manual Python install). Pin inside Python 3.10–3.12 (3.13 blocked by the `cgi` import in `python/server.py`). *macOS freeze landed (#675: `packaging/parse-backend.spec`, onedir build in CI); Windows pending.*
- [ ] Deterministic dependency set produced (lockfile + wheelhouse). *Lockfile landed (#676): `python/requirements.lock.macos-py312.txt` with exact pins, and the macOS workflow installs from it. Wheelhouse not yet produced.*
- [ ] Runtime preflight checks surface missing optional dependencies clearly.
- [x] ffmpeg/ffprobe availability policy is implemented and documented. Resolver: `python/shared/ffmpeg_discovery.py` (`discover_ffmpeg` / `discover_ffprobe`, plus cached `cached_ffmpeg` / `cached_ffprobe` accessors). The server normalize route (`python/server_routes/media.py`), the peaks MP3 fallback (`python/peaks.py`), and the normalize CLI (`python/normalize_audio.py`) all go through the shared discovery policy, so a packaged app that bundles ffmpeg off-PATH works over HTTP, not just from the standalone CLI. The standalone CLI utilities (`video_sync.py`, `video_clip_extract.py`, `export_review_data.py`, `batch_reextract.py`) also resolve through the shared policy since #681, with explicit `--ffmpeg` kept as the highest-priority override; the remaining follow-up is DRYing the per-CLI resolver boilerplate into a shared helper (issue #694). Discovery order and env vars are documented in the "ffmpeg/ffprobe discovery policy" note below.
...
- [ ] **Registry core + read routes.** Model registry scans both roots, parses `manifest.json` (schema_version 1), and resolves stage → model by the precedence in §9.4.2. `GET /api/models` + `GET /api/models/{id}`. Backward-compat guarantee holds: with no models root, stage loading is exactly as today (config `model_path` / HF repo id). *Design: decided. Build: landed (#684 — `python/ai/model_registry.py` scan/resolve, read routes, additive loader wiring).*
- [ ] **Install + per-project binding + job-tracking.** `POST /api/models/install` (202 + `{jobId}`; multipart pack OR `{hfRepoId, stage, format}`), `DELETE /api/models/{id}` (user only), `GET`/`POST /api/models/binding`. Install is job-tracked per the long-running-endpoint rule (registered at both dispatch sites in `python/server_routes/jobs.py`, per-unit `_set_compute_progress`). HF-repo installs generate a manifest and store under the user root. Integrity checks run on unpack/download. *Design: decided. Build: storage + API landed; **per-project binding is now consulted at compute time** — both stage-resolution call sites (`LocalWhisperProvider` stt/ortho, `Aligner.load` ipa) read `model_install.read_binding(<cwd project root>)` and pass it to `resolve_stage_model(stage, binding_id=...)`, guarded so any failure falls back to the pre-binding single-installed-or-`None` behavior (architecture §9.4). This is what makes "install two models for one stage and pick one per project" work. Frontend Settings manager landed (#686; next item).*
- [ ] **Settings > Models manager (frontend).** Installed-models list (name, stage, size, source badge, bundled-vs-user), Add control (upload pack OR paste HF repo id → job-tracked progress via `startCompute`/`pollCompute`), Remove (user models only), per-project stage-assignment dropdowns (STT/IPA/ORTH). First-run prompt to download the standard STT pack when no STT model is installed. *Design: decided. Build: landed (#686 — `src/api/contracts/models.ts` + `src/stores/modelStore.ts` + `src/components/settings/ModelsManager.tsx`); the first-run STT fetch remains pending (see the packaging item below).*
- [ ] **Packaging: bundle IPA into Resources + first-run STT fetch.** Ship the wav2vec2 IPA model read-only under app Resources `models/`; Electron shell sets `PARSE_BUNDLED_MODELS=<resourcesPath>/models` at launch (mirrors `PARSE_BUNDLED_BIN`). First run with no STT model installed fetches the standard STT pack through the install pipeline. *Design: decided. Build: bundle path landed (env wiring + extraResources + CI fetch); first-run STT fetch pending (frontend).*
  - Env wiring: the launcher resolves the read-only models dir from the packaged flag + resourcesPath (`desktop/backend-launcher.js::resolveBundledModelsDir` → `<resourcesPath>/models` when packaged, `undefined` in dev), and the supervisor injects `PARSE_BUNDLED_MODELS` into the backend spawn env **only** when that dir is provided — omitting the key entirely (never an empty string) in dev, so the web/dev product is unchanged (`desktop/backend-supervisor.js`). Covered by `desktop/test/bundled-models.test.js`.
  - Packaging path: `desktop/electron-builder.yml` copies the staged model dir via `extraResources` (`from: ../dist/bundled-models` → `to: models`), landing each pack at `Resources/models/<id>/manifest.json` — the layout `python/ai/model_registry.py` scans.
  - CI staging: `.github/workflows/desktop-macos.yml` runs `desktop/scripts/fetch-bundled-models.py` after Python deps install and before the DMG build, `actions/cache`-keyed on the model id so the ~1.2GB download is not repeated; the packaged-app smoke (`desktop/scripts/smoke-packaged-backend.sh`) asserts `Resources/models/<id>/manifest.json` exists in the built `.app`, proving the model bundled. This grows the DMG (~684MB → ~1.9GB) by design (hybrid delivery bundles only the IPA core).
  - Remaining: first-run STT fetch (no STT model installed → download the standard pack through the install pipeline) is the frontend's job — see the "Settings > Models manager (frontend)" increment above; tracked separately.
...
- [ ] STT model is selected per project, not hardcoded to `razhan/whisper-base-sdh` (satisfied by the per-project stage → model binding in the model registry, architecture §9.4.2; the binding is now consulted at compute time by the stage-resolution call sites, so a per-project selection actually takes effect — remaining work is the Settings dropdown that writes the binding).
