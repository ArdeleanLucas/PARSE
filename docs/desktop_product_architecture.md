...
**Build status (storage + API + binding consumption landed):** the model-registry read/resolve core (`python/ai/model_registry.py`) plus the write side (`python/ai/model_install.py` + `python/server_routes/models.py`) are in place: `POST /api/models/install` (job-tracked, pack-upload or HF download), `DELETE /api/models/{id}` (user models only), and `GET`/`POST /api/models/binding` (per-project stage→model binding persisted under the `project.json` `models` key). **The per-project binding is now consulted at compute time** (the prior open follow-up): both stage-resolution call sites — `LocalWhisperProvider` (stt/ortho, `python/ai/providers/local_whisper.py`) and `Aligner.load` (ipa, `python/ai/forced_align.py`) — read the active project's binding via `model_install.read_binding(<project root>)` and pass it to `resolve_stage_model(stage, binding_id=...)`. The project root is the process cwd (mirroring `server._project_root()`); it is read lazily and via cwd rather than by importing the server, to avoid a circular import. The binding lookup is fully guarded: any failure (import error, corrupt/missing `project.json`, unexpected exception) falls back to `binding_id=None`, i.e. exactly the pre-binding behavior. This is what makes "install two models for one stage and pick one per project" work — with multiple same-stage models installed, `resolve_stage_model` returns `None` (ambiguous) unless the binding disambiguates. **Additive/web-unchanged guarantee holds:** when no binding is set for a stage (the common case, and always true for the web product whose `project.json` has no `models` key, or has no `project.json` at all), `read_binding` returns `None` for that stage → `binding_id=None` → resolution is exactly the single-installed-or-`None` behavior; when no model roots exist at all (the web product), `resolve_stage_model` returns `None` regardless of any binding, so provider/aligner behavior is byte-identical to today.
