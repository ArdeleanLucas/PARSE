---
name: parse-mcp-tool-export-nexus
description: "Use PARSE MCP tool `export_nexus`: Export a NEXUS cognate-character matrix for BEAST2 / phylogenetic tools. Characters are (concept, cognate group) pairs; values are 1/0/? per speaker. Without outputPath returns a preview (first 2000 chars); with outputPath writes inside the project."
version: 1.0.0
...
# PARSE MCP Tool Skill — `export_nexus`

Use this portable skill when calling, validating, reviewing, or documenting the PARSE MCP tool `export_nexus` for any research project, speaker set, language, or corpus hosted in PARSE.

> Source of truth: generated from `python/external_api/catalog.py::build_mcp_http_catalog(..., mode="all")` on `2026-05-10T17:37:02Z`. Re-discover the live schema before execution because tool contracts can evolve.

## Tool contract snapshot

- **Tool name:** `export_nexus`
- **Skill name:** `parse-mcp-tool-export-nexus`
- **Family:** `chat`
- **Mutability:** `mutating`
- **Supports dry-run:** `Yes — `dryRun``
- **Required inputs:** None
- **`additionalProperties`:** `False`
- **Catalog description:** Export a NEXUS cognate-character matrix for BEAST2 / phylogenetic tools. Characters are (concept, cognate group) pairs; values are 1/0/? per speaker. Without outputPath returns a preview (first 2000 chars); with outputPath writes inside the project.
...
- The project must contain enough cognate/enrichment data to build a NEXUS character matrix. (`project_state`, `required`)
...
```bash
curl "$PARSE_BASE_URL/api/mcp/tools/export_nexus?mode=active"
```

## Workflow

1. **Discover** – Confirm `export_nexus` is exposed by the active MCP catalog and inspect its current `inputSchema`.
