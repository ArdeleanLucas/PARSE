...
`--bare` is stricter in current Claude builds than older docs implied: it skips hooks, LSP, plugin sync, attribution, auto-memory, background prefetches, keychain reads, and `CLAUDE.md` auto-discovery, and sets `CLAUDE_CODE_SIMPLE=1`. Anthropic auth in bare mode must come from `ANTHROPIC_API_KEY` or an `apiKeyHelper` provided via `--settings`; OAuth and keychain credentials are not read. Skills still resolve via `/skill-name`, but any other context must be provided explicitly.
...
| CLAUDE.md / extra repo context | `--add-dir <paths...>` |
| Settings / `apiKeyHelper` | `--settings <file-or-json>` |
...
| `--effort <level>` | CLI values: `low`, `medium`, `high`, `max`. Current `claude` v2.1.96 rejects `auto` for the flag; use `/effort auto` interactively if you want automatic effort selection. |
...
| `--bare` | Minimal mode: skips hooks, LSP, plugin sync, attribution, auto-memory, background prefetches, keychain reads, and `CLAUDE.md` auto-discovery; provide auth/context explicitly with flags like `--settings`, `--add-dir`, and `--mcp-config`. |
...
1. **Backticks and special chars in `-p` prompts cause bash command substitution failures** — When passing a prompt string directly via `terminal(command="claude -p '...' ...")`, any backtick `` ` `` in the prompt triggers bash command substitution, corrupting the entire command. CSS `var(--bg)` references, JS closing braces `}())`, and code fences all cause this. **Fix:** write the prompt content (code blocks, CSS, JS) to temp files first, copy to a Windows-accessible path, then use a Python patcher script that reads from those files. Pass only a short natural-language instruction string via `-p`.

   ```bash
   # BAD — backticks and CSS var() corrupt the prompt
   claude -p 'Add this CSS: .foo{background:var(--bg)}'

   # GOOD — write content to files, keep -p prompt short
   # 1. write_file('/tmp/css.txt', '.foo{background:var(--bg)}')
   # 2. write_file('/tmp/patcher.py', 'reads /tmp/css.txt, injects into target')
   # 3. cp /tmp/*.txt /tmp/patcher.py /mnt/c/.../project/
   # 4. python.exe patcher.py
   # 5. claude -p 'Verify the injected CSS compiles' --max-turns 2
   ```

   Note: Windows `python.exe` cannot read `/tmp/` (WSL path) — always copy to a Windows-native path before running with python.exe.
...
10. **Vite proxy `localhost` → IPv6 hang:** When Vite proxies to a backend that only binds `127.0.0.1` (e.g., Python `uvicorn`), using `target: 'http://localhost:PORT'` causes a silent hang — Node resolves `localhost` to `::1` first, TCP connects, but no HTTP response comes (curl exits 28). Fix: use `target: 'http://127.0.0.1:PORT'` and add `ws: false` to all proxy entries unless WebSocket proxying is needed.
10. **Slash commands (like `/commit`) only work in interactive mode** — in `-p` mode, describe the task in natural language instead.
11. **`npx tsc` may resolve to a system stub, not the project's TypeScript** — On some Linux setups, `npx tsc` prints "This is not the tsc command you are looking for" and exits with error because a global shim intercepts it before the local install. Always invoke the project-local binary directly: `./node_modules/.bin/tsc --noEmit`. Add this as a `check` script in `package.json` so it's consistent: `"check": "./node_modules/.bin/tsc --noEmit"`.
12. **Claude Code will stray into neighbouring directories if not explicitly fenced** — When delegating a task scoped to one directory (e.g., `src/hooks/`), Claude Code may create files in adjacent directories (e.g., `src/components/compare/`) if the prompt mentions them as context. "Do NOT touch X" alone is not reliable — confirmed to fail 4 times in a row in a multi-agent repo before the fix below was found.

   **Effective fix — the Boundary Guard pattern.** Add this block at the **END** of every prompt, after all other instructions:

   ```
   BOUNDARY GUARD: After writing all files, run `ls <forbidden_dir>/` — if anything other than .gitkeep exists, delete it immediately before reporting.
   ```

   Example (PARSE Track A must not touch `src/components/compare/`):
   ```
   BOUNDARY GUARD: run `ls src/components/compare/` — if anything other than .gitkeep exists, delete it immediately.
   ```

   The self-check at the end of the prompt is what makes it work — Claude verifies and self-corrects before reporting success. Without it, violations are only caught after the fact. With it, zero violations from first use onwards. Also always check `git status` after completion to catch any boundary violations that slipped through.
13. **`&` inside the terminal command string + `background=true` = orphaned subprocess** — If you put `&` at the end of a shell command AND set `background=true` on the terminal tool, the outer shell forks the process and exits immediately (exit 0), while the inner process runs detached and unmonitorable. The process tool's session captures the shell exit, not the child. Rule: never use `&` inside the command string when `background=true` is set — pick one or the other.
14. **`--bare` is more aggressive than just "skip OAuth"** — current builds also skip auto-memory, keychain reads, plugin sync, background prefetches, and `CLAUDE.md` auto-discovery. Provide auth via `ANTHROPIC_API_KEY` or an `apiKeyHelper`, then pass required context explicitly with flags like `--add-dir`, `--settings`, and `--mcp-config`.
...
4. **Set `--max-turns` in print mode** — prevents infinite loops and runaway costs. Default 15 is too low for complex multi-file JS/HTML fixes (reading + patching 8+ locations across compare.html hits the limit). Use 25 for any task touching inline HTML monoliths or multi-section refactors. `error_max_turns` in the JSON result = increase and retry.
