...
def test_build_post_clef_clear_response_clears_all_forms_preserves_meta_and_backs_up(tmp_path: pathlib.Path) -> None:
    config_path = _seed_config(tmp_path, _sample_clef_payload())
    original = _sample_clef_payload()
    ...
    assert written["ar"]["name"] == "Arabic"
    assert written["ar"]["family"] == "Semitic"
    assert written["ar"]["script"] == "Arab"
    assert written["_meta"] == original["_meta"]
...
def test_build_post_clef_clear_response_language_scope_preserves_other_languages(tmp_path: pathlib.Path) -> None:
    ...
    assert written["_meta"] == _sample_clef_payload()["_meta"]
...
def test_build_post_clef_clear_response_clear_cache_removes_only_known_cache_entries(tmp_path: pathlib.Path) -> None:
    ...
    assert response.status == HTTPStatus.OK
    assert response.payload["summary"]["cacheFilesRemoved"] == 4
...
def test_build_post_clef_clear_response_real_clear_with_no_forms_still_creates_backup(tmp_path: pathlib.Path) -> None:
    config_path = _seed_config(
        tmp_path,
        {
            "_meta": {"primary_contact_languages": ["ar"]},
            "ar": {"name": "Arabic", "script": "Arab", "concepts": {}},
        },
    )
    ...
    response = build_post_clef_clear_response(
        {"dryRun": False},
        config_path=config_path,
        now_factory=lambda: datetime(2026, 4, 28, 20, 51, tzinfo=timezone.utc),
    )

    assert response.status == HTTPStatus.OK
    assert response.payload["summary"]["formsRemoved"] == 0
    assert _read_config(config_path)["_meta"] == {"primary_contact_languages": ["ar"]}
    backups = list(config_path.parent.glob("sil_contact_languages.json.*.bak"))
    assert len(backups) == 1
    assert backups[0].name == "sil_contact_languages.json.20260428T205100Z.bak"


def test_build_post_clef_clear_response_warns_when_language_has_no_concepts_section(tmp_path: pathlib.Path) -> None:
    config_path = _seed_config(
        tmp_path,
        {
            "_meta": {"primary_contact_languages": ["ar"]},
    ...
    response = build_post_clef_clear_response(
        {"dryRun": True, "languages": ["ar"]},
        config_path=config_path,
        now_factory=lambda: datetime(2026, 4, 28, 20, 52, tzinfo=timezone.utc),
    )

    assert response.status == HTTPStatus.OK
    assert response.payload["warnings"] == [
        "Language ar has no concepts object in config/sil_contact_languages.json; no forms removed for that language.",
        "No CLEF reference forms found in config/sil_contact_languages.json for the requested scope.",
    ]
