...
try:
    ...
    from pylexibank_provider import PylexibankProvider  # type: ignore
...
def _stub_provider() -> PylexibankProvider:
    provider = PylexibankProvider()
    provider._load_installed_datasets = lambda: [_FakeDataset()]  # type: ignore[assignment]
    return provider


def test_fetch_for_arabic_excludes_substring_collisions(monkeypatch):
    monkeypatch.setattr(
        "compare.providers.pylexibank_provider.PYLEXIBANK_AVAILABLE",
        True,
    )
