...
def test_import_merges_source_fields_and_custom_order(tmp_path, monkeypatch):
    existing = [
        ...
        {"id": "3", "concept_en": "eyelid", "source_item": "", "source_survey": "", "custom_order": ""},
    ]
    ...
    assert rows_by_id["3"]["source_item"] == ""  # unchanged


def test_import_matches_by_label_when_id_missing(tmp_path, monkeypatch):
    existing = [{"id": "1", "concept_en": "hair", "source_item": "", "source_survey": "", "custom_order": ""}]
...
def test_import_adds_new_concepts(tmp_path, monkeypatch):
    existing = [{"id": "1", "concept_en": "hair", "source_item": "", "source_survey": "", "custom_order": ""}]
    upload = "id,concept_en,source_item,source_survey\n99,sky,3.1,JBIL\n"
    result, rows = _invoke(tmp_path, monkeypatch, existing, upload)
    ...
    assert ids == ["1", "99"]
    assert rows[1]["source_item"] == "3.1"
    assert rows[1]["source_survey"] == "JBIL"


def test_import_replace_mode_clears_unmatched(tmp_path, monkeypatch):
    existing = [
        {"id": "1", "concept_en": "hair", "source_item": "1.1", "source_survey": "KLQ", "custom_order": "10"},
        {"id": "2", "concept_en": "forehead", "source_item": "1.2", "source_survey": "KLQ", "custom_order": "20"},
    ]
    ...
    # Replace clears all optional source/order fields on every existing row, then merges upload values.
    # Upload did not provide source fields for row 1, so they stay cleared.
...
def test_workspace_config_surfaces_source_fields_and_custom_order(tmp_path, monkeypatch):
    (tmp_path / "project.json").write_text("{}", encoding="utf-8")
    with open(tmp_path / "concepts.csv", "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["id", "concept_en", "source_item", "source_survey", "custom_order"])
        w.writeheader()
        w.writerow({"id": "1", "concept_en": "hair", "source_item": "1.1", "source_survey": "KLQ", "custom_order": "10"})
        w.writerow({"id": "2", "concept_en": "forehead", "source_item": "", "source_survey": "", "custom_order": ""})

    monkeypatch.setattr(server, "_project_root", lambda: tmp_path)
    cfg = server._workspace_frontend_config({})
    concepts = cfg["concepts"]
    assert concepts[0]["id"] == "1"
    assert concepts[0]["source_item"] == "1.1"
    assert concepts[0]["source_survey"] == "KLQ"
    assert concepts[0]["custom_order"] == 10
    assert "source_item" not in concepts[1]
    assert "source_survey" not in concepts[1]
    assert "custom_order" not in concepts[1]


def test_import_rewrites_legacy_two_column_concepts_with_full_source_schema(tmp_path, monkeypatch):
    monkeypatch.setattr(server, "_project_root", lambda: tmp_path)
    concepts_path = tmp_path / "concepts.csv"
    concepts_path.write_text("id,concept_en\n1,hair\n2,forehead\n", encoding="utf-8")

    result, rows = _invoke(tmp_path, monkeypatch, None, "id,concept_en\n1,hair\n")

    assert result["ok"] is True
    assert list(rows[0]) == ["id", "concept_en", "source_item", "source_survey", "custom_order"]
