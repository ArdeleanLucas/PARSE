...
def test_create_mcp_server_exposes_64_parse_tools_by_default_without_config(tmp_path, monkeypatch) -> None:
    ...
    assert payload["result"]["mcpToolCount"] == 72
    assert payload["result"]["parseChatToolCount"] == 68
...
def test_create_mcp_server_explicit_false_config_preserves_legacy_curated_surface(tmp_path, monkeypatch) -> None:
    ...
    assert payload["result"]["defaultParseMcpToolCount"] == 68
