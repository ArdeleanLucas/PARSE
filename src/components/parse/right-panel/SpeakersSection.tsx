...
export function SpeakersSection({
  ...
  const [menu, setMenu] = useState<{ speaker: string; x: number; y: number } | null>(null);
  ...
  return (
    <>
          ...
          {speakers.map((speaker) => {
            ...
            return (
              <button
                ...
                onContextMenu={onDeleteSpeaker ? (event) => {
                  event.preventDefault();
                  event.stopPropagation();
                  setMenu({ speaker, x: event.clientX, y: event.clientY });
                } : undefined}
                title={onDeleteSpeaker ? 'Right-click to delete this speaker' : undefined}
      ...
      {menu && onDeleteSpeaker && (
        <div
          role="menu"
          data-testid="speaker-context-menu"
          style={{ position: 'fixed', left: menu.x, top: menu.y, zIndex: 1100 }}
          onClick={(event) => event.stopPropagation()}
          className="min-w-[10rem] rounded-md border border-slate-200 bg-white py-1 text-[11px] shadow-lg dark:border-slate-700 dark:bg-slate-800"
        >
          <button
            type="button"
            role="menuitem"
            data-testid="speaker-context-delete"
            onClick={() => {
              const target = menu.speaker;
              setMenu(null);
              onDeleteSpeaker(target);
            }}
