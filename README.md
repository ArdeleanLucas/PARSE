...
- Built-in AI workflow assistant powered by **58 PARSE-specific tools**
- Full MCP server mode with a **58-tool** default task surface, **62** default adapter tools including workflow macros and `mcp_get_exposure_mode`, concept-scoped pipeline run modes, and an explicit legacy opt-out to the prior curated **38-tool** task surface / **42-tool** adapter surface via `config/mcp_config.json` → `{ "expose_all_tools": false }`
...
- Source-item-aware concept sidebar sorting and badges from `concepts.csv` `source_item` / `source_survey` metadata
- A/B/C realization review in speaker forms with canonical per-speaker picks persisted through `manual_overrides.canonical_realizations`
- Manual concept merge/unmerge overrides for comparative review, stored in `manual_overrides.concept_merges` without rewriting `concepts.csv` or annotation tiers
...
- Compute jobs can be cooperatively cancelled with `POST /api/compute/{jobId}/cancel`; ORTH persists partial output with `partial_cancelled` metadata when cancellation lands mid-run
- Startup and `POST /api/locks/cleanup` perform non-destructive stale `*.lock` cleanup without killing processes
...
- [AI Integration](docs/ai-integration.md) — providers, model configuration, the 58-tool chat surface, and workflow macros.
