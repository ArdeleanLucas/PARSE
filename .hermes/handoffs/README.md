...
This directory is the local-only coordinator-owned task queue for PARSE lanes. Prompt handoff files are intentionally ignored by git and should not be committed to PARSE; only this convention README remains tracked. Other `.hermes` runtime/agent artifacts such as automation state and parity report logs are also local-only.
...
- Runtime/agent evidence artifacts under `.hermes/reports/` and `.hermes/automation/state/` are also ignored/local-only.
